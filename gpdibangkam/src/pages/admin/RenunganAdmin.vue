<template>
        ...
        <div class="rounded-t mb-0 px-4 py-3 border-0 bg-blue-700">
          <div class="flex flex-wrap items-center">
            <div class="relative w-full px-4 max-w-full flex-grow flex-1">
              <h3 class="font-semibold text-lg text-white">Data Renungan</h3>
            </div>
          </div>
        </div>
        ...
        <div class="relative overflow-x-auto lg:px-10">
          <table class="table-fixed w-full text-sm text-left text-white">
            ...
            <tbody v-if="paginatedData.length > 0" class="text-gray-600">
              <tr
                v-for="(renungan, index) in paginatedData"
                :key="renungan.id"
                :class="[
                  isSelected(renungan)
  ...
  <inputModal
    ...
    title="Tambah Data Renungan"
    btnTitle="Tambah Data"
    :inputFields="fieldsTambahModal"
    urlEndpoint="renungan"
    urlMethod="POST"
...
const filters = computed(() => {
  ...
  rawApiData.value.forEach((renungan) => {
    for (const key in renungan) {
      if (!uniqueFilters[key]) {
        uniqueFilters[key] = {};
      }
      const value = renungan[key];
...
const filteredData = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter((renungan) =>
      Object.values(renungan).some((value) =>
  ...
  for (const key in selectedFilters.value) {
    ...
    if (selectedValues.length > 0) {
      filtered = filtered.filter((renungan) =>
        selectedValues.includes(renungan[key])
      );
    }
  }
  return filtered;
...
onMounted(() => {
  instance.emit("route-changed", "Data Renungan");
