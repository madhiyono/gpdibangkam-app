      ...
      >
        <div class="rounded-t mb-0 px-4 py-3 border-0 bg-blue-700">
          <div class="flex flex-wrap items-center">
            <div class="relative w-full px-4 max-w-full flex-grow flex-1">
              <h3 class="font-semibold text-lg text-white">Data Pernikahan</h3>
            </div>
          </div>
        </div>
        <div class="grid lg:grid-cols-3 grid-cols-1 gap-4 px-4 lg:px-10 py-5">
          ...
          <div class="text-sm font-medium text-gray-900 bg-white">
            <div
              :class="[isAsalPasanganCollapse ? 'border border-gray-400' : '']"
              class="flex items-center justify-between p-2"
            >
              <span class="font-semibold text-lg">Filter Asal Pasangan</span>
              <button
                class="flex items-center justify-center px-2 text-gray-400 hover:bg-gray-200"
                @click="collapseAsalPasanganHandle"
              >
                <i
                  v-if="isAsalPasanganCollapse"
            ...
            <ul
              :class="[isAsalPasanganCollapse ? 'hidden' : 'flex flex-col']"
              class="w-full border max-h-48 overflow-y-auto border-gray-200"
            >
              <li
                v-for="(count, value) in filters.asal_jemaat_kedua"
                :key="value"
                class="w-full"
              >
                <input
                  ...
                  type="checkbox"
                  class="hidden peer"
                  :checked="isFilterSelected('asal_jemaat_kedua', value)"
                  @change="toggleFilter('asal_jemaat_kedua', value)"
                  :value="value"
                />
                <label
                  :for="`option-${value}`"
          ...
          <div class="text-sm font-medium text-gray-900 bg-white">
            <div
              ...
              class="flex items-center justify-between p-2"
            >
              <span class="font-semibold text-lg">Filter Status</span>
              <button
                class="flex items-center justify-center px-2 text-gray-400 hover:bg-gray-200"
        ...
        <div class="relative overflow-x-auto lg:px-10">
          <table class="table-fixed w-full text-sm text-left text-white">
            <thead class="text-xs uppercase bg-blue-700 text-center">
              <th class="w-16">No.</th>
              <th class="px-6 py-3">Nama Pihak Pertama</th>
              <th class="px-6 py-3">Tempat / Tanggal Lahir</th>
              <th class="px-6 py-3">Nama Pihak Kedua</th>
              <th class="px-6 py-3">Asal Pihak Kedua</th>
              <th class="px-6 py-3">Tempat / Tanggal Lahir</th>
            ...
            <tbody v-if="paginatedData.length > 0" class="text-gray-600">
              ...
              >
                ...
                <td class="text-center px-2">
                  {{ data.tempat_lahir_jemaat_pertama }},
                  {{ data.tanggal_lahir_jemaat_pertama }}
                </td>
                <td class="text-center px-2">
                  {{ data.nama_jemaat_kedua }}
                </td>
                <td class="text-center px-2">
                  {{ data.asal_jemaat_kedua }}
                </td>
                <td class="text-center px-2">
                  {{ data.tempat_lahir_jemaat_kedua }},
                  {{ data.tanggal_lahir_jemaat_kedua }}
                </td>
                <td class="text-center px-2">
                  {{ data.status }}
                </td>
            ...
            <tbody v-else class="text-gray-600">
              <tr class="border-b select-none bg-white hover:bg-blue-200 h-12">
                ...
                <td v-else colspan="9" class="text-center font-semibold">
...
//#region Filtering
const isAsalPasanganCollapse = ref(true);

function collapseAsalPasanganHandle() {
  isAsalPasanganCollapse.value = !isAsalPasanganCollapse.value;
}
...
onMounted(() => {
  instance.emit("route-changed", "Data Pernikahan");
...
function loadData() {
  ...
  useFetch("GET", "pernikahan", null, authStore.accessToken)
    .then((responsePernikahan) => {
      responsePernikahan.data
        .sort((a, b) => {
        ...
        .map((nikah) => {
          if (nikah.id_pelaksana) {
            useFetch(
              "GET",
              `users/jemaat/${nikah.id_pelaksana}`,
              null,
              authStore.accessToken
            ).then((responsePelaksana) => {
              nikah.nama_pelaksana = responsePelaksana.data.nama_lengkap;
            });
          } else {
            nikah.nama_pelaksana = null;
          }

          useFetch(
            "GET",
            `users/jemaat/${nikah.id_jemaat_pertama}`,
            null,
            authStore.accessToken
          ).then((responseJemaatPertama) => {
            const dateTanggalLahirPertama = new Date(
              responseJemaatPertama.data.tanggal_lahir
            );
            nikah.tanggal_lahir_jemaat_pertama =
              dateTanggalLahirPertama.toLocaleDateString("id-ID", options);
            nikah.tempat_lahir_jemaat_pertama =
              responseJemaatPertama.data.tempat_lahir;
            nikah.nama_jemaat_pertama = responseJemaatPertama.data.nama_lengkap;

            if (nikah.asal_jemaat_kedua === "Jemaat") {
              useFetch(
                "GET",
                `users/jemaat/${nikah.id_jemaat_kedua}`,
                null,
                authStore.accessToken
              ).then((responseJemaatKedua) => {
                const dateTanggalLahirKedua = new Date(
                  responseJemaatKedua.data.tanggal_lahir
                );
                nikah.tanggal_lahir_jemaat_kedua =
                  dateTanggalLahirKedua.toLocaleDateString("id-ID", options);
                nikah.tempat_lahir_jemaat_kedua =
                  responseJemaatKedua.data.tempat_lahir;
                nikah.nama_jemaat_kedua = responseJemaatKedua.data.nama_lengkap;
              });
            } else {
              useFetch(
                "GET",
                `jemaatluar/${nikah.id_jemaat_kedua}`,
                null,
                authStore.accessToken
              ).then((responseJemaatKedua) => {
                const dateTanggalLahirKedua = new Date(
            ...
            useFetch(
              "GET",
              `pernikahan/catatan/${nikah.id}`,
              null,
              authStore.accessToken
            ).then((responseCatatanPernikahan) => {
              nikah.id_catatan = responseCatatanPernikahan.data[0].id;
              delete responseCatatanPernikahan.data[0].id;
              delete responseCatatanPernikahan.data[0].id_pernikahan;

              Object.entries(responseCatatanPernikahan.data[0]).forEach(
                (input) => {
              ...
              if (nikah.tanggal_nikah !== null) {
                ...
                nikah.raw_tanggal_nikah = moment(dateTanggalNikah)
                  .tz("Asia/Jakarta")
              ...
              if (nikah.tanggal_nikah_cs !== null) {
                ...
                nikah.raw_tanggal_nikah_cs = moment(dateTanggalNikahCS)
                  .tz("Asia/Jakarta")
                  .format("YYYY-MM-DDTHH:mm");
              }
              if (nikah.tanggal_pengumuman_kedua !== null) {
                ...
                nikah.raw_tanggal_pengumuman_kedua = moment(datePengumumanKedua)
                  .tz("Asia/Jakarta")
                  .format("YYYY-MM-DDTHH:mm");
              }
              if (nikah.tanggal_pengumuman_pertama !== null) {
                ...
                nikah.raw_tanggal_pengumuman_pertama = moment(
                  datePengumumanPertama
                )
                  .tz("Asia/Jakarta")
                  .format("YYYY-MM-DDTHH:mm");
              }
              if (nikah.tanggal_proses !== null) {
                ...
                nikah.raw_tanggal_proses = moment(dateProses)
                  .tz("Asia/Jakarta")
                  .format("YYYY-MM-DDTHH:mm");
              }
...
function selectRow(data) {
  if (selectedRows.value.includes(data)) {
    ...
    detailDataModal.value = [
      ...
      { label: "ID Catatan Pernikahan", type: "text", value: data.id_catatan },
      { label: "Created At", type: "text", value: data.created_at },
      { label: "Tanggal Nikah", type: "text", value: data.tanggal_nikah },
      { label: "Nama Pelaksana", type: "text", value: data.nama_pelaksana },
      { label: "Nama Gembala", type: "text", value: data.pengembala_nikah },
      {
        label: "Tanggal Proses Pernikahan",
        type: "text",
        value: data.tanggal_proses,
      },
      {
        label: "Tanggal Pengumuman Pertama",
        type: "text",
        value: data.tanggal_pengumuman_pertama,
      },
      {
        label: "Tanggal Pengumuman Kedua",
        type: "text",
        value: data.tanggal_pengumuman_kedua,
      },
      {
        label: "Tanggal Nikah Catatan Sipil",
        type: "text",
        value: data.tanggal_nikah_cs,
      },
      {
        label: "Nama Pengurus Catatan Sipil",
        type: "text",
        value: data.nama_pelaksana_cs,
      },
      {
        label: "Surat Keterangan Kelurahan",
        type: "downloadfile",
        value: data.surat_keterangan_kelurahan,
      },
      {
        label: "Surat Keterangan Belum Menikah",
        type: "downloadfile",
        value: data.surat_keterangan_belum_menikah,
      },
      {
        label: "Foto Berpasangan",
        type: "downloadfile",
        value: data.foto_berpasangan,
      },
      {
        label: "Nama Pihak Pertama",
        type: "text",
        value: data.nama_jemaat_pertama,
      },
      {
        label: "Tempat / Tanggal Lahir Pihak Pertama",
        type: "text",
        value: `${data.tempat_lahir_jemaat_pertama}, ${data.tanggal_lahir_jemaat_pertama}`,
      },
      {
        label: "Nama Ayah Pihak Pertama",
        type: "text",
        value: data.ayah_jemaat_pertama,
      },
      {
        label: "Agama Ayah Pihak Pertama",
        type: "text",
        value: data.agama_ayah_jemaat_pertama,
      },
      {
        label: "Nama Ibu Pihak Pertama",
        type: "text",
        value: data.ibu_jemaat_pertama,
      },
      {
        label: "Agama Ibu Pihak Pertama",
        type: "text",
      ...
      },
      {
        ...
        type: "text",
        value: data.nama_jemaat_kedua,
      },
      {
        label: "Asal Pihak Kedua",
        type: "text",
        value: data.asal_jemaat_kedua,
      },
      {
        label: "Tempat / Tanggal Lahir Pihak Kedua",
        type: "text",
        value: `${data.tempat_lahir_jemaat_kedua}, ${data.tanggal_lahir_jemaat_kedua}`,
      },
      {
        label: "Nama Ayah Pihak Kedua",
        type: "text",
        value: data.ayah_jemaat_kedua,
      },
      {
        label: "Agama Ayah Pihak Kedua",
        type: "text",
        value: data.agama_ayah_jemaat_kedua,
      },
      {
        label: "Nama Ibu Pihak Kedua",
        type: "text",
        value: data.ibu_jemaat_kedua,
      },
      {
        label: "Agama Ibu Pihak Kedua",
        type: "text",
      ...
      {
        label: "Asal Gereja Saksi Pertama",
        type: "text",
        value: data.asal_gereja_saksi_pertama,
      },
      { label: "Nama Saksi Kedua", type: "text", value: data.nama_saksi_kedua },
      {
        label: "Asal Gereja Saksi Kedua",
        type: "text",
...
function uploadSertifikat(event) {
  if (event.target.files[0].type === "application/pdf") {
    ...
    useFetch(
      "PUT",
      `pernikahan/upload/${selectedRows.value[0].id}`,
      formData,
      authStore.accessToken
    )
      .then(() => {
    ...
    return [
      {
        ...
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].raw_tanggal_nikah,
        defaultValue: selectedRows.value[0].raw_tanggal_nikah,
      },
      {
      ...
      {
        label: "Nama Gembala (Pisahkan Dengan Koma)",
        name: "pengembala_nikah",
        type: "text",
        placeholder: "Nama Pengembala Nikah",
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].pengembala_nikah,
        defaultValue: selectedRows.value[0].pengembala_nikah,
      },
      {
        label: "Tanggal Proses Pernikahan",
        name: "tanggal_proses",
        ...
        isDisable: false,
        isRequire: true,
        value: selectedRows.value[0].raw_tanggal_proses,
        defaultValue: selectedRows.value[0].raw_tanggal_proses,
      },
      {
        label: "Tanggal Pengumuman Pertama",
        name: "tanggal_pengumuman_pertama",
        type: "datetime-local",
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].raw_tanggal_pengumuman_pertama,
        defaultValue: selectedRows.value[0].raw_tanggal_pengumuman_pertama,
      },
      {
        label: "Tanggal Pengumuman Kedua",
        name: "tanggal_pengumuman_kedua",
        type: "datetime-local",
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].raw_tanggal_pengumuman_kedua,
        defaultValue: selectedRows.value[0].raw_tanggal_pengumuman_kedua,
      },
      {
        label: "Nama Pengurus Catatan Sipil",
        name: "nama_pelaksana_cs",
        type: "text",
        placeholder: "Nama Pengurus Catatan Sipil",
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].nama_pelaksana_cs,
        defaultValue: selectedRows.value[0].nama_pelaksana_cs,
      },
      {
        label: "Tanggal Nikah Catatan Sipil",
        name: "tanggal_nikah_cs",
        type: "datetime-local",
        isDisable: false,
        isRequire: false,
        value: selectedRows.value[0].raw_tanggal_nikah_cs,
        defaultValue: selectedRows.value[0].raw_tanggal_nikah_cs,
      },
      {
        label: "Status",
        name: "status",
