<template>
  ...
  <deleteModal
    ...
    title="Hapus Data Penyerahan Anak"
    urlEndpoint="penyerahananak"
    :urlId="idData"
...
onMounted(() => {
  instance.emit("route-changed", "Data Penyerahan Anak");
...
function loadData() {
  ...
  useFetch("GET", "penyerahananak", null, authStore.accessToken)
    .then((responsePenyerahanAnak) => {
      responsePenyerahanAnak.data
        .sort((a, b) => {
        ...
        .map((anak) => {
          if (anak.id_pelaksana) {
            useFetch(
              "GET",
              `users/jemaat/${anak.id_pelaksana}`,
              null,
              authStore.accessToken
            ).then((responsePelaksana) => {
              anak.nama_pelaksana = responsePelaksana.data.nama_lengkap;
            });
          } else {
            anak.nama_pelaksana = null;
          }
...
function selectRow(data) {
  if (selectedRows.value.includes(data)) {
    ...
    detailDataModal.value = [
      ...
      { label: "Pas Foto", type: "image", value: data.pas_foto },
      {
        label: "Akta Kelahiran",
        type: "downloadfile",
        value: data.akta_kelahiran,
      },
...
function uploadSertifikat(event) {
  if (event.target.files[0].type === "application/pdf") {
    ...
    useFetch(
      "PUT",
      `penyerahananak/upload/${selectedRows.value[0].id}`,
      formData,
      authStore.accessToken
    )
      .then(() => {
